...
def create_non_colliding_groups():
	result = ""
	group_index = 0
	if "non_colliding_group" in arms:
		for pair in arms["non_colliding_group"]:
			new_group = f"""//Generated by Armature
	unordered_map<dGeomID, int> no_collision_map{group_index};
	"""
			result = f'{result}{new_group}'
			for group in pair:
				for shape in pair[group]:
					new_shape = f"no_collision_map{group_index}[geom_{shape}] = 0;\n	"
					result = f'{result}{new_shape}'
			result = f'{result}\n	no_collision_groups.push_back(no_collision_map{group_index});\n'
			group_index += 1
